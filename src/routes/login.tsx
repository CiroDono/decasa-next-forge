...
function LoginPage() {
  ...
  const onDniChange = (v: string) => setDni(v.replace(/\D/g, "").slice(0, 9));
  const onTelChange = (v: string) => {
    const cleaned = v.replace(/[^\d+]/g, "");
    setTelefono(cleaned.startsWith("+") ? "+" + cleaned.slice(1).replace(/\+/g, "") : cleaned.replace(/\+/g, ""));
  };
  const onEmailChange = (v: string) => setEmail(v.trim().toLowerCase().slice(0, 254));

  const passChecks = useMemo(() => ({
    len: passRegex.len(password),
    upper: passRegex.upper(password),
    lower: passRegex.lower(password),
    digit: passRegex.digit(password),
  }), [password]);
  ...
  const fieldErrors = useMemo(() => {
    ...
    const r = registerSchema.safeParse({ nombre, email, dni, telefono, password });
    if (r.success) return {};
    const errs: Record<string, string> = {};
    r.error.issues.forEach((i) => { if (!errs[i.path[0] as string]) errs[i.path[0] as string] = i.message; });
    return errs;
  }, [mode, email, password, nombre, dni, telefono]);
